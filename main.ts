namespace SpriteKind {
    export const pro2 = SpriteKind.create()
    export const boss = SpriteKind.create()
    export const sis = SpriteKind.create()
    export const bproj = SpriteKind.create()
}
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    projectile = sprites.createProjectileFromSprite(img`
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . 2 2 . . . . . . . 
        . . . . . . 3 1 1 3 . . . . . . 
        . . . . . 2 1 1 1 1 2 . . . . . 
        . . . . . 2 1 1 1 1 2 . . . . . 
        . . . . . . 3 1 1 3 . . . . . . 
        . . . . . . . 2 2 . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        `, mySprite, 50, 0)
})
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile`, function (sprite, location) {
    level += 1
    scene.setBackgroundImage(img`
        
        `)
    tiles.placeOnTile(sprite, tiles.getTileLocation(3, 3))
    if (level == 2) {
        tiles.setTilemap(tilemap`level3`)
        mySprite3 = sprites.create(img`
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            .........................fff................................
            .......................ffaccff..............................
            ......................fcaaacccf.............................
            ......................f555faccf.............................
            .....................fa55faacccffff.........................
            .....................faccaaccfff5f5f........................
            .....................ffffffff55ffffff.......................
            .............fffffffffffffaaf5ff1f555f......................
            .......ffffff55fbbbbbfafaaaff1ff11fff.......................
            ......f11f111f11fbbbbfafaffbf5f51f555f......................
            ......2f11f11f55fbbbbfafaf1bbfffffffff......................
            ......2f11f11f55fbbbbfafaf1bfcaccf1ff.......................
            ......f11f111f55fbbbbfafafffcacccf1f........................
            .......ffffff55ffffffaf555ffaaccf1bf........................
            .............fffaaaaafafffafaacffbbf........................
            ...............fafaafaaaaaafcccfbbbf........................
            ...............fafaaaaaaaaafcccfbbbf........................
            ..............faffaaaaaaaaafffbf11bbff......................
            .............faaffaaffaaaaaaaaf.fb1f55f.....................
            ............faaaaafafbffaaaaaf...ff5aaaf....................
            ..........fffaaaaf.fbbbbfffff....faaaaaf....................
            .........fb1bfaaaf.fffffbbbbf....faaffff....................
            .........f1f1bfff..f5555fbbbf....faf11bf....................
            ........fbff111bf..ffffffffff....ffb1f1f....................
            ........fbf111f1f..faaaaaaaaf...fb1111fbf...................
            ........fbf11bf1f..faaaaaaaaf...f1fb1bfbf...................
            ........ffbbffff...faaaaaaaaf....fffbfbf....................
            .........fbff.....faaaaaaaaaaf......ffbf....................
            ..........f.......faaaaffaaaaf........f.....................
            .................fffaaf..faafff.............................
            ................f555faf..faf555f............................
            ................faa55f....f55aaf............................
            ...............faaaa55f..f55aaaaf...........................
            ...............faaaaaaf..faaaaaaf...........................
            ..............faaaaaaaf..faaaaaaaf..........................
            ..............faaaaaaaf..faaaaaaaf..........................
            .............faaaaaaaaf...faaaaaaaff........................
            .............f55555aaaf...faaa55555f........................
            ............ffffff55af....faa55fffff........................
            ...........fddddddf55f.....f55fdddddf.......................
            ..........fddddddddff......fffdddddddf......................
            .........fdddddddddddf.....fddddddddddf.....................
            ........fddddddddddddff....fdddddddddddf....................
            ........fffffffffffffff....fffffffffffff....................
            ............................................................
            ............................................................
            ............................................................
            `, SpriteKind.boss)
        mySprite4 = sprites.create(img`
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ....................fffffff..fff............................
            ...................f5555555ff555f...........................
            ..................f555555555ff555f..........................
            ..................f55f5f5f555f5555f.........................
            ..................ff5f5f5ff55f5555f.........................
            ..................fdd1d111.ffff555f.........................
            ..................fd1fdff11fdff555f.........................
            ..................fd1fdff11ddf.555f.........................
            ..................fdd1d111dddf.555f.........................
            ..................fdddddddddf..fff..........................
            ...................fd33ddddff...............................
            ...................ffdddddfaf...............................
            ..................ffdfdfffaddf..............................
            ..................fddfdddaaddf..............................
            ..................fddfddddddf...............................
            ..................fdfafadddf................................
            ..................ffffafffaf................................
            ...................faaaaaaaaf...............................
            ..................faaaaaaaaaaf..............................
            ...................ffaaaaaaff...............................
            .....................ffffff.................................
            ...................ffa1f11aff...............................
            ..................faaaafaaaaaf..............................
            ..................ffffffffffff..............................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            ............................................................
            `, SpriteKind.sis)
        tiles.placeOnTile(mySprite3, tiles.getTileLocation(24, 5))
        tiles.placeOnTile(mySprite4, tiles.getTileLocation(31, 2))
        game.splash("its time to finish this!")
    }
})
function boss_fight () {
    if (level == 2) {
        info.startCountdown(30)
        projectile2 = sprites.createProjectileFromSprite(img`
            f f b f f b f f 
            f f b f f b f b 
            b f f f f f f b 
            b f b f f f b f 
            f f f f b f b f 
            f f f f f f f f 
            f f f f b f b f 
            b f b b f f f b 
            `, mySprite3, randint(-20, -50), 0)
    }
}
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    mySprite.vy = -50
})
sprites.onOverlap(SpriteKind.bproj, SpriteKind.Player, function (sprite, otherSprite) {
    info.changeLifeBy(-1)
    if (info.life() == 0) {
        mySprite.say("NOOO ROLL")
        music.wawawawaa.play()
        game.over(false)
    }
})
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.Enemy, function (sprite, otherSprite) {
    otherSprite.say("ow!", 1000)
    otherSprite.destroy(effects.fire, 500)
    info.setScore(10)
    mySprite.say("GET OUT OF THE WAY!")
})
let projectile2: Sprite = null
let mySprite4: Sprite = null
let mySprite3: Sprite = null
let projectile: Sprite = null
let level = 0
let mySprite: Sprite = null
game.splash("HELP SAVE YOUR SISTER.... ", "USE A,B, AND THE ARROW KEYS")
mySprite = sprites.create(img`
    ...................................
    ...............fff.................
    .............ff699ff...............
    ............f8888698f..............
    ...........f888888998f.............
    ...........f888888899f.............
    ..........f888f6682222f............
    ..........f8ff66668212f............
    ..........fff1f6ff6828f............
    ..........ff2ffdd1f686f............
    ...........f2ffdd1ff6f.............
    ..........f8ff9dddfdf..............
    .........f8888fddddddffff..........
    .........f888fffdfddf1111fffff.....
    .........f88f99fddddf111f89998ff...
    ..........ff8699fffff11f86119ff2f..
    ...........ff86f8666881f88999ff2f..
    ...........f8f8f888888f888888ff2f..
    ............f8ff8888ff.ff88888ff...
    ............f888fffff....fffff.....
    ............f8888668f..............
    ............f8888868f..............
    ............f6988888f..............
    ...........f999988889f.............
    ...........ff999f88999f............
    ..........f88f9f.f869f8ff..........
    ..........f896ff..f6f8889f.........
    .........f8699f....f888869f........
    .........f6999f....f8888669f.......
    ........f86999f.....f888666f.......
    ........f86999f.....f8889669f......
    .......f886998f......f888966f......
    ......f8fff89f.......f88886fff.....
    .....f88969f8f.......f8888f999f....
    ....f8896669ff......f8ffff96699f...
    ...f888666669f......f88888896699f..
    ...fffffffffff......fffffffffffff..
    ...................................
    ...................................
    ...................................
    `, SpriteKind.Player)
tiles.placeOnTile(mySprite, tiles.getTileLocation(3, 3))
let mySprite2 = sprites.create(img`
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ..................................................fffff.........
    ................................................ff22222f........
    ...................................ff....fff...f22222552f.......
    ..................................f4f...f444ffffff222255f.......
    ..................................f4f...ff4444f444ff22f44f......
    .................................f4f...f4ff444f48844ff444f......
    .................................f4f..f44f.f444f888844f444f.....
    ................................f4ffff44f...f4fff888884ff4f.....
    ................................f444f44fff..ff44ff8888844f......
    ...............................f4444444444ff4444f8f8888884f.....
    ...............................f4444444444444444488888ffff......
    ...............................f4444f44ff44444444484ff4444f.....
    ..............................f4444f144f.f4444f444f444f44ff.....
    .............................f4444ff14f...f44f1f44f444f44f......
    .............................f444444ff...f4ff4444f44444ff.......
    ..............................f444ff.....f44f144f444444f........
    ...............................fff........f44fff4f44444f........
    .........................................f4ff455f44444f.........
    .........................................ffffff4f44444f.........
    ...............................................fff444f..........
    ................................................f1414f..........
    .................................................ffff...........
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    ................................................................
    `, SpriteKind.Enemy)
tiles.placeOnTile(mySprite2, tiles.getTileLocation(7, 4))
controller.moveSprite(mySprite, 100, 0)
mySprite.ay = 200
scene.cameraFollowSprite(mySprite)
mySprite2.vx = 20
mySprite2.setBounceOnWall(true)
info.setLife(3)
info.setScore(0)
scene.setBackgroundImage(img`
    
    `)
tiles.setTilemap(tilemap`level1`)
level = 1
game.onUpdateInterval(500, function () {
    boss_fight()
})
